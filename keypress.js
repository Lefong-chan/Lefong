...
'.kbf{outline:3px solid #e50914!important;outline-offset:4px!important;box-shadow:0 0 0 6px rgba(229,9,20,.2)!important;position:relative!important;z-index:9999!important;}',
'.kbf *{position:relative!important;z-index:auto!important;}',
'#chgrid .gcard.kbf,.citem.kbf,.chitem.kbf,.src-item.kbf,.add-src-btn.kbf,.sbtab.kbf,.vbtn.kbf,.hbtn.kbf,.navbtn.kbf,.closebtn.kbf,.cbtn.kbf,.fsbtn.kbf,.retrybtn.kbf,.footlink.kbf,.wcard.kbf,.backbtn.kbf{overflow:visible!important;}',
'#num-hud{position:absolute;top:16px;right:16px;z-index:9999;background:rgba(0,0,0,.82);border:2px solid rgba(229,9,20,.7);border-radius:12px;padding:10px 18px 8px;display:flex;flex-direction:column;align-items:center;gap:6px;pointer-events:none;transition:opacity .25s;}',
'#num-hud.hidden{opacity:0;pointer-events:none;}',
'#num-hud-digits{display:flex;gap:8px;align-items:center;}',
'.nhd{width:22px;height:28px;border-bottom:2px solid rgba(255,255,255,.35);display:flex;align-items:center;justify-content:center;font-family:"Barlow Condensed","Barlow",sans-serif;font-size:1.1rem;font-weight:700;color:#fff;letter-spacing:1px;transition:border-color .15s,color .15s;}',
'.nhd.filled{border-color:#e50914;color:#e50914;}',
'#num-hud-hint{font-family:"Barlow Condensed","Barlow",sans-serif;font-size:.62rem;letter-spacing:1.5px;color:rgba(255,255,255,.45);text-transform:uppercase;}'
].join('');
document.head.appendChild(s);
})();

var KBN={cur:null,lastGrid:null,lastClist:null,lastSidebar:null,playerOvTimer:null,numBuf:'',numTimer:null,numHudTimer:null};
...
function ensureNumHud(){
  ...
  if(!h){
    ...
    h.id='num-hud';
    h.innerHTML='<div id="num-hud-digits"></div><div id="num-hud-hint">Ch. direct</div>';
    var pw=document.getElementById('pwrap');
    if(pw)pw.appendChild(h);else pv.appendChild(h);
  }
  return h;
}

function renderNumHud(){
  var h=ensureNumHud();if(!h)return;
  var digs=document.getElementById('num-hud-digits');
  if(!digs)return;
  var buf=KBN.numBuf;
  var cells='';
  for(var i=0;i<5;i++){
    var c=i<buf.length?buf[i]:'_';
    var cls=i<buf.length?'nhd filled':'nhd';
    cells+='<span class="'+cls+'">'+c+'</span>';
  }
  digs.innerHTML=cells;
...
function onKey(e){
  ...
  var mkeys={'MediaPlayPause':1,'MediaTrackNext':1,'MediaTrackPrevious':1,'MediaStop':1};
  if(mkeys[e.key]){
    if(isPV()){
      e.preventDefault();
      if(e.key==='MediaPlayPause')try{togglePlay();}catch(er){}
      else if(e.key==='MediaTrackNext')try{nextCh();}catch(er){}
      else if(e.key==='MediaTrackPrevious')try{prevCh();}catch(er){}
      else if(e.key==='MediaStop')try{stopStream();}catch(er){}
    }
    return;
  }
  ...
  if(e.key==='Delete'||e.key==='Supr'||e.keyCode===46){
    ...
    if(KBN.cur&&KBN.cur.closest('.src-item[data-src-idx]')){
      openDelModal();return;
    }
    return;
  }
...
function handleNumKey(digit){
  ...
  if(KBN.numBuf.length>=5){
    ...
    KBN.numBuf='';
    KBN.numHudTimer=setTimeout(hideNumHud,600);
    if(idx>=0&&typeof channels!=='undefined'&&idx<channels.length)try{playCh(idx);}catch(e){}
    return;
  }
