...
var K={
  c:null,   // current focused element
  g:null,   // last grid card (key press)
  s:null,   // last sidebar element (key press right)
  lc:null,  // last clist item
  pt:null,  // player overlay timer
  nb:'',    // number buffer
  nt:null,  // number timer
  nh:null,  // number hud timer
  nk:0      // debounce timestamp
};
...
// go right: to last visited grid card (or first)
function gR(){
  if(!vis('welcomeScr')){
...
// go up from footer: to last visited grid card
function gFU(){
...
// Badge: channel number while playing
function mkBadge(){
...
function onKey(e){
  var now=Date.now();
  // debounce: ignore repeated keydown within 80ms (TV remote double-fire)
  if(e.repeat||(K.nk&&now-K.nk<80))return;
...
function hCI(c,k){
  ...
  if(k==='ArrowRight'){K.s=c;gR();return;}
  if(k==='ArrowUp'){
    // FIX: "Toutes les chaînes" (index 0) → tabCh
...
function hGC(c,k){
  ...
  if(k==='ArrowLeft'){
    // FIX: first of row → last sidebar element (not tabSrc)
...
function hNum(d){
  ...
  K.nb+=d;rHud();try{showOv();}catch(x){}rPOT();
  if(K.nb.length>=5){
    var idx=parseInt(K.nb)-1;K.nb='';K.nh=setTimeout(hHud,600);
...
// Patch playCh to track last played card
(function(){
  ...
  window.playCh=function(idx){
    ...
    // Find card with this index and store as last grid
    var cs=gGC();
