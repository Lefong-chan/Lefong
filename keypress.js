...
'#ch-badge{position:absolute;top:10px;right:10px;z-index:199;background:rgba(0,0,0,.78);border:1px solid rgba(229,9,20,.5);border-radius:6px;padding:4px 9px;font-family:"Barlow Condensed",sans-serif;font-size:.78rem;font-weight:700;letter-spacing:1px;color:rgba(255,255,255,.85);pointer-events:none}'+
'#ch-badge.cb-hide{display:none}'+
'#num-hud{position:absolute;top:10px;right:10px;z-index:200;background:rgba(0,0,0,.85);border:2px solid rgba(229,9,20,.65);border-radius:10px;padding:9px 16px 7px;display:flex;flex-direction:column;align-items:center;gap:5px;pointer-events:none}'+
...
function kF(el){
  if(!el)return;
  if(KBN.c&&KBN.c!==el){
    KBN.c.classList.remove('kbf');
    if(KBN.c.tagName==='INPUT'||KBN.c.tagName==='TEXTAREA')KBN.c.blur();
  }
  KBN.c=el;
  ...
  try{
    ...
    while(p){
      ...
      if(ov==='auto'||ov==='scroll'){
        ...
        if(r.bottom>pr.bottom-6)p.scrollTop+=r.bottom-pr.bottom+10;
        else if(r.top<pr.top+6)p.scrollTop-=pr.top-r.top+10;
        break;
      }
      p=p.parentElement;
    }
...
function mF(el){
  if(!el)return;
  if(KBN.c)KBN.c.classList.remove('kbf');
  KBN.c=el;el.classList.add('kbf');
...
function gCols(){
  ...
  if(!g||g.classList.contains('list'))return 1;
  var t=getComputedStyle(g).gridTemplateColumns;
  return(t&&t!=='none')?t.split(' ').filter(Boolean).length||1:1;
}

function gCardByIdx(idx){
  var cs=gGC();
  for(var i=0;i<cs.length;i++){
    if(cs[i].getAttribute('onclick')==='playCh('+idx+')')return cs[i];
  }
  return null;
}
...
function gR(){
  if(!isWV()){
    var g=KBN.lG&&document.contains(KBN.lG)?KBN.lG:(gGC()[0]||null);
    if(g){kF(g);return;}
  }
  var w=gWU();if(w)kF(w);
}
...
function mkHud(){
  ...
  if(!h){
    h=document.createElement('div');h.id='num-hud';h.classList.add('nh-hide');
    h.innerHTML='<div id="nhd"></div><div id="nhl">Ch. direct</div>';
    var pw=document.getElementById('pwrap');(pw||pv).appendChild(h);
  }
  return h;
}

function mkBadge(){
  var pw=document.getElementById('pwrap');if(!pw)return null;
  var b=document.getElementById('ch-badge');
  if(!b){b=document.createElement('div');b.id='ch-badge';b.classList.add('cb-hide');pw.appendChild(b);}
  return b;
}
...
function showBadge(){
  ...
  if(typeof curChIdx!=='undefined'&&curChIdx>=0&&typeof channels!=='undefined'&&channels.length){
    b.textContent='Ch. '+(curChIdx+1)+' / '+channels.length;
    b.classList.remove('cb-hide');
  }
}

function hideBadge(){var b=document.getElementById('ch-badge');if(b)b.classList.add('cb-hide');}

function rHud(){
  var h=mkHud();if(!h)return;
  var d=document.getElementById('nhd');if(!d)return;
  var s='';
  for(var i=0;i<5;i++)s+='<span class="nd'+(i<KBN.nb.length?' nf':'')+'>'+(i<KBN.nb.length?KBN.nb[i]:'_')+'</span>';
...
function syncOvBadge(){
  var ov=document.getElementById('playerov');
  if(!ov||ov.classList.contains('hidden')){hideBadge();}
  else{if(isPV()&&!document.getElementById('num-hud')||document.getElementById('num-hud').classList.contains('nh-hide'))showBadge();}
}
...
function onKey(e){
  ...
  if(mk[e.key]){
    if(isPV()){
      e.preventDefault();
      KBN.nk=true;setTimeout(function(){KBN.nk=false;},120);
  ...
  if(e.key==='Delete'||e.key==='Supr'||e.keyCode===46){
    ...
    if(KBN.c&&KBN.c.closest('.src-item[data-src-idx]'))oDM();
    return;
  }
  var nv=['ArrowLeft','ArrowRight','ArrowUp','ArrowDown','Enter'];
  if(nv.indexOf(e.key)===-1){
    if(isPV()&&isFS()&&/^[0-9]$/.test(e.key)){
      KBN.nk=true;setTimeout(function(){KBN.nk=false;},80);
      hNum(e.key);
    }
    return;
  }
  if(isPV()){hPl(e);return;}
  if(!KBN.c||!document.contains(KBN.c)){
    var ts=document.getElementById('tabC');if(ts)kF(ts);
  ...
  if(k==='ArrowDown'){
    if(iS){
      if(isTA('s')){var s=gSI();if(s.length){kF(s[0]);return;}var b=document.querySelector('.add-src-btn');if(b){kF(b);return;}var r=gRI();if(r.length)kF(r[0]);}
        ...
        if(isTA('c')&&!isSub()){var ci=gCL();if(ci.length){kF(ci[0]);return;}}
        if(isTA('c')&&isSub()){var bk=document.getElementById('backToGroups');if(bk){kF(bk);return;}}
        var f=document.querySelector('.footlinks .footlink');if(f)kF(f);
      }
      return;
    }
    if(iC){
      if(isTA('c')){
        if(!isSub()){var ct=gCL();if(ct.length){kF(ct[0]);return;}}
        if(isSub()){var bk2=document.getElementById('backToGroups');if(bk2){kF(bk2);return;}}
        var f2=document.querySelector('.footlinks .footlink');if(f2)kF(f2);
...
function hSI(c,k){
  ...
  if(k==='ArrowRight'){KBN.lSb=c;gR();return;}
...
function hAB(k){
  ...
  if(k==='ArrowRight'){KBN.lSb=document.querySelector('.add-src-btn');gR();return;}
...
function hVG(k){
  ...
  if(k==='ArrowDown'){if(KBN.lG&&document.contains(KBN.lG)){kF(KBN.lG);return;}var g=gGC()[0]||null;if(g)kF(g);return;}
...
function hGC(c,k){
  ...
  if(k==='ArrowLeft'){
    ...
    if(i===0||fR){
      if(KBN.lSb&&document.contains(KBN.lSb)){kF(KBN.lSb);return;}
      if(KBN.lC&&document.contains(KBN.lC)){kF(KBN.lC);return;}
      var ts=document.getElementById('tabC');if(ts)kF(ts);
      return;
    }
    kF(cs[i-1]);return;
  ...
  if(k==='ArrowUp'){var ab=i-cols;if(ab<0){var vb=Array.from(document.querySelectorAll('.vbtn'));var inv=vb.find(function(b){return!b.classList.contains('active');});if(inv)kF(inv);return;}kF(cs[ab]);return;}
...
function hHD(k){
  ...
  if(k==='ArrowDown'){var vb=Array.from(document.querySelectorAll('.vbtn'));var inv=vb.find(function(b){return!b.classList.contains('active');});if(inv&&isGV()){kF(inv);return;}var w=gWU();if(w)kF(w);return;}
...
function rPOT(){
  clearTimeout(KBN.pT);
  KBN.pT=setTimeout(function(){hideOv();hideBadge();},5000);
}
...
function hNum(d){
  ...
  if(KBN.nb.length>=5){
    var idx=parseInt(KBN.nb)-1;KBN.nb='';
    KBN.nHT=setTimeout(hHud,600);
    if(idx>=0&&typeof channels!=='undefined'&&idx<channels.length)try{playCh(idx);}catch(x){}
    return;
  }
  KBN.nT=setTimeout(function(){
    var idx=parseInt(KBN.nb)-1;KBN.nb='';
    KBN.nHT=setTimeout(hHud,600);
    if(idx>=0&&typeof channels!=='undefined'&&idx<channels.length)try{playCh(idx);}catch(x){}
  },3000);
...
function hCMod(e){
  ...
  if(e.key==='ArrowLeft'||e.key==='ArrowRight'||e.key==='Tab'){
    e.preventDefault();
    var nx=(e.key==='ArrowLeft'||(e.key==='Tab'&&e.shiftKey))?(ci<=0?bt.length-1:ci-1):(ci>=bt.length-1?0:ci+1);
    mF(bt[nx]);return;
  }
...
(function(){
  var _origPlayCh=window.playCh;
  window.playCh=function(idx){
    if(typeof _origPlayCh==='function')_origPlayCh.apply(this,arguments);
    var g=gCardByIdx(idx);
    if(g)KBN.lG=g;
    setTimeout(function(){showBadge();},150);
  };

  var _origShowOv=window.showOv,_origHideOv=window.hideOv;
  if(_origShowOv){
    window.showOv=function(){
      if(typeof _origShowOv==='function')_origShowOv.apply(this,arguments);
      setTimeout(showBadge,10);
    };
  }
  if(_origHideOv){
    window.hideOv=function(){
      if(typeof _origHideOv==='function')_origHideOv.apply(this,arguments);
      hideBadge();
    };
  }
  ...
  function rA(){
    hHud();hideBadge();KBN.nb='';clearTimeout(KBN.nT);clearTimeout(KBN.nHT);
    setTimeout(function(){
...
document.addEventListener('DOMContentLoaded',function(){
  setTimeout(function(){if(!KBN.c){var t=document.getElementById('tabC');if(t)kF(t);}},350);
});
