/* ============================================================
   KEY NAVIGATION v2 — CORRECTIONS COMPLÈTES
   ============================================================
   Focus outline: rouge (#e50914).
   ← ↑ → ↓ = déplacement uniquement.
   Enter/Ok = activation.
   Input text : clavier affiché seulement au passage (blur sinon).
   ============================================================ */

/* ── CSS focus rouge injecté ───────────────────────────────── */
...
/* ── État navigation ───────────────────────────────────────── */
const KBN = {
  cur:            null,   // élément focalisé courant
  lastGrid:       null,   // dernière gcard sélectionnée (Enter)
  lastClist:      null,   // dernier citem sélectionné
  playerOvTimer:  null,   // timer auto-hide overlay player
};

/* ── kbFocus : applique la marque rouge ────────────────────── */
function kbFocus(el) {
  if (!el) return;
  if (KBN.cur && KBN.cur !== el) {
    KBN.cur.classList.remove('kbf');
    /* FIX: désactiver les inputs quand on les quitte */
  ...
  /* Inputs : focus DOM pour afficher le clavier au passage */
...
/* ── goRight : vers la zone droite (grille ou welcome) ──────── */
function goRight() {
  if (!isWelcomeVisible()) {
...
/* ── goFootUp : depuis footer ↑ → lastGrid (FIX) ───────────── */
function goFootUp() {
  /* FIX: toujours lastGrid si dispo, jamais le dernier card */
...
/* ── Ouvre modale suppression depuis srcItem focalisé ───────── */
