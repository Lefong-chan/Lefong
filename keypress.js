// keypress.js — Smart TV optimized
(function(){
  // Inject styles once only
...
var K={
  c:null,   // focused element
  g:null,   // last grid card
  s:null,   // last sidebar element
  lc:null,  // last clist item
  pt:null,  // player overlay timer
  nb:'',    // number buffer (up to 5 digits)
  nt:null,  // number input timer
  nh:null,  // hud hide timer
  _hudReady:false
};
  ...
  try{
    ...
    while(p){
      ...
      if(o==='auto'||o==='scroll'){
        ...
        if(r.bottom>pr.bottom-4)p.scrollTop+=r.bottom-pr.bottom+12;
        else if(r.top<pr.top+4)p.scrollTop-=pr.top-r.top+12;
        break;
      }
      p=p.parentElement;
    }
...
function mF(el){
  if(!el)return;
  if(K.c)K.c.classList.remove('kbf');
  K.c=el;el.classList.add('kbf');
...
// Voaforona indray mandeha fotsiny (tsy misy doublon)
function mkHud(){
  var pw=$i('pwrap');if(!pw)return null;
  if(K._hudReady)return $i('nh');
...
// ── Player overlay timer ───────────────────────────────────
function rPOT(){
  clearTimeout(K.pt);
  K.pt=setTimeout(function(){
    try{hideOv();}catch(e){}
    hideBadge();
  },5000);
}
...
// ══════════════════════════════════════════════════════════
function onKey(e){
  // Smart TV: ignore auto-repeat only (pas de debounce timestamp — cause du bug "2× press")
  if(e.repeat)return;
  ...
  if(mk[e.key]){
    if(isPV()){
      ...
      else if(e.key==='MediaStop'){safeStop();}
    }
    return;
  }
  ...
  // ── Chiffres: fullscreen + player → changement de chaîne direct ──
...
// ══════════════════════════════════════════════════════════
// hNum — saisie chiffre direct (1 touche = affiché immédiatement)
// 5 chiffres max — 3s après dernier chiffre → change chaîne
// ══════════════════════════════════════════════════════════
function hNum(d){
  ...
  // Ajoute le chiffre immédiatement
  K.nb+=d;

  // Affiche le HUD immédiatement (pas de délai)
  rHud();
  try{showOv();}catch(x){}
  rPOT();

  // Si 5 chiffres atteint → change chaîne immédiatement
  if(K.nb.length>=5){
    var nb=K.nb;
    K.nb='';
  ...
  // Sinon attend 3 secondes après le dernier chiffre saisi
...
function oDM(){
  ...
  var i=parseInt(it.dataset.srcIdx);
  if(!isNaN(i))try{delSource({stopPropagation:function(){}},i);}catch(e){}
}
...
// ── Safe stop (Smart TV: no crash/quit) ───────────────────
function safeStop(){
  // Reset state first
  K.nb='';
  clearTimeout(K.nt);clearTimeout(K.nh);clearTimeout(K.pt);
  hHud();hideBadge();
  // Remove focus safely
  if(K.c){K.c.classList.remove('kbf');K.c=null;}
  // Call original functions safely
  try{stopStream();}catch(x){
    // Fallback minimal si stopStream crash
    try{
      var v=document.getElementById('vid');
      if(v){v.pause();v.src='';}
    }catch(e2){}
    try{closePlayer();}catch(e3){}
  }
}
...
// ── Patch playCh / showOv / hideOv / closePlayer / stopStream ──
(function(){
  ...
  if(_oSO)window.showOv=function(){
    _oSO.apply(this,arguments);
    setTimeout(showBadge,10);
  };
  if(_oHO)window.hideOv=function(){
    _oHO.apply(this,arguments);
    hideBadge();
  };

  var _oC=window.closePlayer,_oS=window.stopStream;

  // Restore focus after close/stop
  function rA(){
    setTimeout(function(){
  ...
  window.closePlayer=function(){
    if(typeof _oC==='function'){try{_oC.apply(this,arguments);}catch(x){}}
    rA();
  };
  window.stopStream=function(){
    if(typeof _oS==='function'){try{_oS.apply(this,arguments);}catch(x){}}
    rA();
  };
})();
