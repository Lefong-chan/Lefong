...
'#ch-badge.h{display:none}'+
'#nh{position:absolute;top:10px;right:10px;z-index:200;background:rgba(0,0,0,.85);border:2px solid rgba(229,9,20,.65);border-radius:10px;padding:9px 16px 7px;display:flex;flex-direction:column;align-items:center;gap:5px;pointer-events:none}'+
'#nh.h{display:none}'+
...
function gR(){
  if(!vis('welcomeScr')){
    var g=K.g&&document.contains(K.g)?K.g:gGC()[0]||null;
    if(g){kF(g);return;}
  }
  var w=gWU();if(w)kF(w);
}
...
function gFU(){
  var g=K.g&&document.contains(K.g)?K.g:null;
  if(g){kF(g);return;}
...
function mkBadge(){
  ...
  var b=$i('ch-badge');
  if(!b){b=document.createElement('div');b.id='ch-badge';b.className='h';pw.appendChild(b);}
  return b;
}
function showBadge(){
  var b=mkBadge();if(!b)return;
  if(typeof curChIdx!=='undefined'&&curChIdx>=0&&typeof channels!=='undefined'&&channels.length){
    b.textContent='Ch.'+(curChIdx+1)+' / '+channels.length;b.classList.remove('h');
  }
}
function hideBadge(){var b=$i('ch-badge');if(b)b.className='h';}

// Number HUD — 5 chiffres
function mkHud(){
  var pw=$i('pwrap');if(!pw)return null;
  var h=$i('nh');
  if(!h){h=document.createElement('div');h.id='nh';h.className='h';h.innerHTML='<div id="nhd"></div><div id="nhl">Ch. direct</div>';pw.appendChild(h);}
  return h;
}
function rHud(){
  ...
  // 5 chiffres
  var s='';for(var i=0;i<5;i++)s+='<span class="nd'+(i<K.nb.length?' f':'')+'>'+(i<K.nb.length?K.nb[i]:'_')+'</span>';
  d.innerHTML=s;h.classList.remove('h');hideBadge();
}
function hHud(){var h=$i('nh');if(h)h.className='h';}

function rPOT(){clearTimeout(K.pt);K.pt=setTimeout(function(){try{hideOv();}catch(e){}hideBadge();},5000);}

function onKey(e){
  var now=Date.now();
  // debounce Smart TV: ignore keydown within 120ms
  if(e.repeat||(K.nk&&now-K.nk<120))return;
  ...
  if(mk[e.key]){
    if(isPV()){
      ...
      if(e.key==='MediaPlayPause')try{togglePlay();}catch(x){}
        ...
        // Smart TV: stop safe — no crash
        K.nb='';clearTimeout(K.nt);clearTimeout(K.nh);
        try{stopStream();}catch(x){}
      }
    }
    return;
  }
  ...
  // Chiffre: fullscreen + player → channel direct
  if(isPV()&&isFS()&&/^[0-9]$/.test(e.key)){
    K.nk=now;e.preventDefault();hNum(e.key);return;
  }
...
function hST(c,k){
  ...
  if(k==='ArrowDown'){
    ...
    if(iS){
      if(isTA('c')&&!isSub()){var ci=gCL();if(ci.length)return kF(ci[0]);}
      if(isTA('c')&&isSub()){var bk=$i('backToGroups');if(bk)return kF(bk);}
      var f=$q('.footlinks .footlink');if(f)kF(f);return;
    }
    if(iC&&isTA('c')){
      if(!isSub()){var ct=gCL();if(ct.length)return kF(ct[0]);}
      var bk2=$i('backToGroups');if(bk2)return kF(bk2);
      var f2=$q('.footlinks .footlink');if(f2)kF(f2);return;
    }
...
function hGC(c,k){
  ...
  if(k==='ArrowLeft'){
    if(i===0||(cols>1&&i%cols===0)){
      if(K.s&&document.contains(K.s)){kF(K.s);return;}
      if(K.lc&&document.contains(K.lc)){kF(K.lc);return;}
      kF($i('tabC'));return;
    }
    kF(cs[i-1]);return;
  }
  ...
  if(k==='ArrowUp'){
    var ab=i-cols;
    if(ab<0){var vb=$a('.vbtn');var inv=vb.find(function(b){return!b.classList.contains('active');});if(inv)kF(inv);return;}
    kF(cs[ab]);return;
  }
  if(k==='ArrowDown'){
    var bl=i+cols;
    if(bl>=cs.length){var f=$q('.footlinks .footlink');if(f)kF(f);return;}
    kF(cs[bl]);return;
  }
...
// ============================================================
// hNum — channel direct par chiffre
// 5 chiffres max
// 1 chiffre → miasa avy hatrany (timeout 800ms)
// ============================================================
function hNum(d){
  ...
  // Raha feno 5 chiffres → miasa avy hatrany
  if(K.nb.length>=5){
    var nb5=K.nb;
    K.nb='';
    K.nh=setTimeout(hHud,500);
    var idx5=parseInt(nb5)-1;
    if(idx5>=0&&typeof channels!=='undefined'&&idx5<channels.length){
      try{playCh(idx5);}catch(x){}
    }
    return;
  }

  // Timeout: 800ms — haingana, indray manindry miasa
...
// Patch playCh + stopStream/closePlayer — Smart TV safe
(function(){
  ...
  window.playCh=function(idx){
    ...
    for(var i=0;i<cs.length;i++){
      if(cs[i].getAttribute('onclick')==='playCh('+idx+')'){K.g=cs[i];break;}
    }
  ...
  // Smart TV: reset state BEFORE calling original → tsy misy crash
  function rA(){
    hHud();hideBadge();
    K.nb='';clearTimeout(K.nt);clearTimeout(K.nh);clearTimeout(K.pt);
    var old=K.c;
    K.c=null;
    if(old)old.classList.remove('kbf');
  ...
  window.closePlayer=function(){
    rA();
    if(typeof oC==='function'){try{oC.apply(this,arguments);}catch(x){}}
  };

  window.stopStream=function(){
    rA();
    if(typeof oS==='function'){try{oS.apply(this,arguments);}catch(x){}}
  };
})();
...
document.addEventListener('DOMContentLoaded',function(){
  setTimeout(function(){if(!K.c){var t=$i('tabC');if(t)kF(t);}},350);
});
